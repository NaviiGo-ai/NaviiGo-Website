...
from services.itinerary_engine import fetch_destination_data_with_gemini, DEST_DATA_VERSION
...
def _is_stale_version(data: Dict[str, Any]) -> bool:
    """Check if cached data was generated by an older prompt version."""
    return data.get("_v", 0) < DEST_DATA_VERSION


async def _background_refetch(key: str, dest_name: str, purpose: str, budget: int, days: int):
    """Re-fetch destination data from Gemini in background and update all cache layers."""
    try:
        print(f"[Cache] Background re-fetch for {dest_name} (stale v{DEST_DATA_VERSION - 1} → v{DEST_DATA_VERSION})...")
        fresh_data = await fetch_destination_data_with_gemini(dest_name, purpose, budget, days)
        if fresh_data:
            _mem_set(key, fresh_data)
            await _file_set(key, fresh_data)
            print(f"[Cache] Background re-fetch complete for {dest_name} — now v{DEST_DATA_VERSION}")
    except Exception as e:
...
async def get_destination_data(
    ...
    Version-aware: if cached data has an older schema version, it is returned
    immediately (no user-facing delay) but a background task re-fetches fresh
    data from Gemini so the next request gets the enriched version.
    """
    ...
    if data:
        ...
        if _is_stale_version(data):
            asyncio.create_task(_background_refetch(key, dest_name, purpose, budget, days))
        return data
    ...
    if data:
        ...
        print(f"[Cache] Stored {dest_name} in all cache layers (v{DEST_DATA_VERSION})")
        return data
