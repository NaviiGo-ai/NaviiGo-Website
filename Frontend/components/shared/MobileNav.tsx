        ...
        <div className="p-6 pt-5 space-y-6">
          ...
          <div className="space-y-2">
            {[
              ...
              <div key={item.name}>
                <motion.div
                  className="flex items-center justify-between px-4 py-3 rounded-lg hover:bg-gradient-to-r hover:from-saffron/10 hover:to-indigo/10 text-slate-800 hover:text-saffron transition-colors dark:text-slate-200 dark:hover:text-indigo"
                      ...
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                        <div className="pl-8 pr-2 py-1 space-y-1">
                          {ITINERARY_DROPDOWN.map((sub) => (
                            <Link key={sub.name} href={sub.href} onClick={toggleMenu} className="flex items-center gap-3 px-3 py-2 rounded-lg text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-white/5 hover:text-saffron dark:hover:text-indigo transition-colors">
